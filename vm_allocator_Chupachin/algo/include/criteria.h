...
public:
    ...
    inline static bool elementIncidentLinksDescending(const Element * first, const Element * second) {
        return first->sumAdjacentLinks() > second->sumAdjacentLinks();
    }

    inline static bool elementIncidentLinksAndWeightDescending(const Element * first, const Element * second) {

        double weightLinks = 0.9;
        double weightParams = 0.1;
        return weightParams * first->weight() + weightLinks * first->sumAdjacentLinks() >
                weightParams * first->weight() + weightLinks * second->sumAdjacentLinks();
    }
    ...
    inline static bool elementIncidentLinksAscending(const Element * first, const Element * second) {
        return first->sumAdjacentLinks() < second->sumAdjacentLinks();
    }
