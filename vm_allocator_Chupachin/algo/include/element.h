...
protected:    
    ...
    virtual bool physicalCheck(const Element * other) const {
        ...
        for ( ; it != parameters.end(); ++it ) {
            ...
            if (type->getName() == std::string("links")) {
                value = new ParameterInt(sumAdjacentLinks());
            }
...
public:
    Element() : type(NONE), physical(false),
        available(true), attributes(0), assignee(0), value(1.0), affinity(false), anti_affinity(false)
    {}
